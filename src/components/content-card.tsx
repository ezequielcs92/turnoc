...
export function ContentCard({ index, eyebrow, title, copy, href, tags = [] }: ContentCardProps) {
  return <Link className="content-card" href={href} data-reveal><div><div className="card-number" aria-hidden="true">{String(index + 1).padStart(2, "0")}</div><div className="card-meta"><span>{eyebrow}</span>{tags.slice(0, 3).map((tag) => <span className="pill" key={tag}>{tag}</span>)}</div><h2 className="card-title">{title}</h2><p className="card-copy">{copy}</p></div><span className="card-arrow" aria-hidden="true">↗</span></Link>;
}
